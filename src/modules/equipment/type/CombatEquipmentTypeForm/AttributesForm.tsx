...
export const AttributesForm = observer((props: AttributesFormProps) => {
  ...
  const tableData: ReactNode[][] =
    props.combatEquipmentTypeFormStore.getAttributes().map((attribute) => {
      return [
        <p>{attribute?.name || '—'}</p>,
        <p>{attribute?.value || '—'}</p>,
        <IconButton onClick={() => props.combatEquipmentTypeFormStore.deleteAttribute(attribute?.name ?? '')}>
  ...
  const addAttribute = () => props.combatEquipmentTypeFormStore.addAttribute(attributeName, attributeValue)
  ...
  return (
    <>
      ...
      <RowWrapper>{props.combatEquipmentTypeFormStore.getAttributes().length > 0 && <Table columns={tableColumns} data={tableData} />}</RowWrapper>
    </>
  )
})
