...
export const AttributesForm = observer((props: AttributesFormProps) => {
  ...
  const tableData: ReactNode[][] =
    props.weaponTypeFormStore.getAttributes().map((attribute) => {
      return [
        <p>{attribute?.name || '—'}</p>,
        <p>{attribute?.value || '—'}</p>,
        <IconButton onClick={() => props.weaponTypeFormStore.deleteAttribute(attribute?.name ?? '')}>
  ...
  const addAttribute = () => props.weaponTypeFormStore.addAttribute(attributeName, attributeValue)
  ...
  return (
    <>
      ...
      <RowWrapper>{props.weaponTypeFormStore.getAttributes().length > 0 && <Table columns={tableColumns} data={tableData} />}</RowWrapper>
    </>
  )
})
